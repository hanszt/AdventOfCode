...
public class Tile {
    ...
    public Tile getNeighborByInstruction(final String instruction, final Map<Point2D, Tile> allTiles) {
        ...
        final Point2D newPosition = new Point2D(this.position.getX() + delta.getX(), this.position.getY() + delta.getY());
        final Tile neighbor;
    ...
    void countBlackNeighbors(final Map<Point2D, Tile> allTiles) {
        ...
        for (final Point2D delta : INSTRUCTION_TO_DIR.values()) {
            final Point2D neighborPosition = new Point2D(this.position.getX() + delta.getX(), this.position.getY() + delta.getY());
    ...
    List<Tile> neighbors() {
        ...
        for (final Point2D delta : INSTRUCTION_TO_DIR.values()) {
            neighbors.add(new Tile(new Point2D(position.getX() + delta.getX(), position.getY() + delta.getY())));
        }
        return neighbors;
    }
    ...
    public String toString() {
        return String.format("Tile{position='(x=%3d, y=%3d)', blackUp=%-5b, nrOfBlackNeighbors=%d, Neighbors={%s}}",
                position.getX(), position.getY(), blackUp, nrOfBlackNeighbors, neighborsAsString());
    }
}
