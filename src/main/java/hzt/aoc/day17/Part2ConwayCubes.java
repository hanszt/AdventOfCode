...
public class Part2ConwayCubes extends Part1ConwayCubes {
            ...
            for (int z = 0; z < grid3d.size(); z++) {
                ...
                for (int y = 0; y < grid2d.size(); y++) {
                    ...
                    for (int x = 0; x < row.size(); x++) {
                        final int activeNeighbors = countActiveNeighbors(new Point4D(x, y, z, w), grid4d);
    ...
    private int countActiveNeighbors(final Point4D cur, final List<List<List<List<Boolean>>>> curGrid4d) {
    ...
    private static boolean isActiveNeighbor(final Point4D checked, final Point4D cur, final List<List<List<List<Boolean>>>> curGrid4d) {
        return !cur.equals(checked) && curGrid4d.get(checked.getW()).get(checked.getZ()).get(checked.getY()).get(checked.getX());
    }
